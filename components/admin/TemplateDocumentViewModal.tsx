...
type TemplateDocumentViewModalProps = {
  title: string
  previewUrl: string
  onClose: () => void
...
export function TemplateDocumentViewModal({
  title,
  previewUrl,
  onClose,
  onError,
}: TemplateDocumentViewModalProps) {
    ...
    >
      <div className={styles.popup} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <div>
            <h2 id="template-view-title" className={styles.title}>
              {title}
            </h2>
        ...
        <div className={styles.body}>
          ...
          {!loading && !error && preview?.format === 'pdf' && pdfUrl && (
            <iframe title={title} src={pdfUrl} className={styles.pdfFrame} />
          )}
