...
export async function POST(request: Request) {
  try {
    ...
    if (!orderId || !productId || !notes) {
      return NextResponse.json(
        { error: 'Order, product, and issue description are required' },
        { status: 400 }
      )
    }

    if (!agreed) {
      return NextResponse.json({ error: 'You must agree to the warranty terms' }, { status: 400 })
    }
    ...
    if (order.status === 'cancelled') {
      return NextResponse.json({ error: 'Cancelled orders cannot file warranty claims' }, { status: 400 })
    }

    if (order.payment_status !== 'paid') {
      return NextResponse.json({ error: 'Warranty claims require a paid order' }, { status: 400 })
    }
    ...
    const msg = e instanceof Error ? e.message : 'Failed to submit warranty claim'
